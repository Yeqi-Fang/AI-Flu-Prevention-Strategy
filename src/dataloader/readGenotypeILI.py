...
def read_and_validate_excel(file_path: str) -> pd.DataFrame:
    ...
    required_columns = ['year', 'week', 'genotype', 'ILIp']
...
def calculate_weekly_averages(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # 计算每个genotype在每个week的ILIp均值
    weekly_averages = df.groupby(['genotype', 'week'])['ILIp'].mean().reset_index()

    # 按照40-52周和1-39周排列
    weekly_averages['week'] = weekly_averages['week'].astype(int)
    weekly_averages = weekly_averages.sort_values(by=['genotype', 'week'])
...
def rearrange_weeks_by_genotype(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # 对每个genotype独立处理
    genotype_groups = df.groupby('genotype')
    rearranged_dfs = []
...
def read_and_validate_genotypeILI_excel(file_path: str) -> pd.DataFrame:
    try:
...
if __name__ == "__main__":
    excel_file_path = "allILI.xlsx"

    try:
        df = read_and_validate_genotypeILI_excel(excel_file_path)
